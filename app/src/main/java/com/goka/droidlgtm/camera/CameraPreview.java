...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, Camera.AutoFocusCallback {
    ...
    public static enum CameraMode {
        BACK(0), //  Back camera (ID = 0)
        FRONT(1); // Front camera (ID = 1)
    ...
    public CameraPreview(Activity activity, CameraMode cameraMode, LayoutMode mode) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            if (Camera.getNumberOfCameras() > cameraMode.id) {
                mCameraMode = cameraMode;
            } else {
                mCameraMode = CameraMode.BACK;
            }
        } else {
            mCameraMode = CameraMode.BACK;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            mCamera = Camera.open(mCameraMode.id);
        } else {
            mCamera = Camera.open();
        }
    ...
    protected Camera.Size determinePreviewSize(boolean portrait, int reqWidth, int reqHeight) {
        ...
        if (Config.DEBUG) {
            Logger.LOGV(LOG_TAG, "Listing all supported preview sizes");
            for (Camera.Size size : mPreviewSizeList) {
                Log.v(LOG_TAG, "  w: " + size.width + ", h: " + size.height);
            }
            Logger.LOGV(LOG_TAG, "Listing all supported picture sizes");
    ...
    protected Camera.Size determinePictureSize(Camera.Size previewSize) {
        ...
        Logger.LOGV(LOG_TAG, "Same picture size not found.");
    ...
    protected boolean adjustSurfaceLayoutSize(Camera.Size previewSize, boolean portrait,
        ...
        Logger.LOGV(LOG_TAG, "Preview Layout Size - w: " + layoutWidth + ", h: " + layoutHeight);
        Logger.LOGV(LOG_TAG, "Scale factor: " + fact);
    ...
    protected void configureCameraParameters(Camera.Parameters cameraParams, boolean portrait) {
        ...
        Logger.LOGV(LOG_TAG, "Preview Actual Size - w: " + mPreviewSize.width + ", h: " + mPreviewSize.height);
        Logger.LOGV(LOG_TAG, "Picture Actual Size - w: " + mPictureSize.width + ", h: " + mPictureSize.height);
